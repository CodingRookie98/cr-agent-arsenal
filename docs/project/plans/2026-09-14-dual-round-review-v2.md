...
- **当前执行通道**: Heavy Track
- **当前活跃阶段**: P3 TDD 循环（阶段 B）
- **当前活跃子任务**: B-1
- **当前子任务重试计数**: 0/3
- **外层循环迭代**: 0/5
- **最后一次验证状态**: L0 全绿 —— 残留宿主绑定 0 命中；断链扫描 0；`bash -n` OK
- **最新有效提交**: 待 A-4 回填
- **阻断原因**: 无
...
### 阶段 B：结构对齐
- [ ] **B-1 维度条件化**：将 React/SSR/Hooks/Storage 维度改为按 diff 文件特征激活；R1/R2 输出模板的对应字段改为条件存在
- [ ] **B-2 Diff 边界锁单一真相源**：在 `verdict-rubric.md` 定义一次，SKILL.md/templates/catalog/脚本改为引用
- [ ] **B-3 消费方路径 + R2 上下文对称**：脚本调用改技能目录相对写法；R2 获得与 R1 对等的仓库读取授权，驳回必须附 `文件:行` 证据，新增"批量降级需证据"红旗
- [ ] **B-4 去否定化**：把可正向化的禁令改写为正向目标，收敛为 invariants 清单
- [ ] **B-5 提交 B**

### 阶段 C：平台化与可验证
- [ ] **C-1 输出契约机器可解析化**：R1/R2 报告与 Previous Blockers 增加稳定 ID 与统一 schema
- [ ] **C-2 脚本增强**：`--help`/usage、写入审查记录 scaffold、可选 `--prompt` 产出可直接派发的提示词
- [ ] **C-3 脚本单测 + 门禁**：新增 `tests/`，并把断链/残留断言纳入技能自检
- [ ] **C-4 文档同步与验收**：README/index 同步，跑通第 3 节全部命令
- [ ] **C-5 提交 C**
