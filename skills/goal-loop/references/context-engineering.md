...
* **物理隔离**：所有的关键架构决策、接口契约、技术债排查、环境状态、当前执行步骤与完成标记，必须持久化到计划文件（唯一可读真相源）及其检查点锚点，临时细节写入 `.goal-loop/scratchpad.md`，严禁仅保存在模型 Short-Term 记忆内。
...
| **系统中断/会话崩溃重启** | 🔄 **全量恢复读** | 主动读取计划文件及其检查点锚点，依据已勾选的 `- [x]` 与宿主 goal 状态秒级恢复执行指针。 |
...
```mermaid
graph TD
    Resume(["会话重启或中断恢复"]) --> Step1["第 1 步: 探测工作区计划文件<br/>检查 docs/project/plans/*.md 及其检查点锚点"]
...
1. **第 1 步**：检查工作区中的计划文件（`docs/project/plans/*.md`）及其检查点锚点；
...
- **当前活跃子任务**: [如：Task P3.2 - 导出纯函数级联取消实现]
