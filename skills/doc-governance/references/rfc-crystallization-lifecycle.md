...
> ⚠️ **本段是评审的前置条件（术语先行）**：先用统一术语澄清需求与边界。图中所示"同一事物的多个名字被理解为多个实体，进而生成原本不存在的需求"即源于术语未先对齐。
> 范围以「核心术语 + **易产生歧义**」为界，**禁止扩张为全量词表**。

### 5.1 核心术语定义 (Core Term Definitions)
| 术语 | 定义（一句话，可判定） | 范围 / 边界 | 不包含 (Excludes) |
|---|---|---|---|
| [如：客户] | [如：已完成实名认证并开通结算账户的主体] | [如：仅指已开通结算账户的主体] | [如：未开户的访客与潜在用户] |

### 5.2 易歧义与同义词裁决 (Ambiguity & Synonym Resolution)
| # | 候选称谓 | 裁决采用的**唯一**术语 | 区别与裁决依据 | 裁决人 | 日期 |
|---|---|---|---|---|---|
| T1 | [如：客户 / 用户 / 会员 / 账号] | [如：客户] | [四者在既有代码与业务口述中的实际差异；为何只保留其一] | [用户] | YYYY-MM-DD |

### 5.3 禁用称谓与历史别名 (Deprecated Aliases)
| # | 禁用/历史称谓 | 状态 | 替代术语 | 备注 |
|---|---|---|---|---|
| A1 | [如：会员] | [禁用 / 历史遗留] | [客户] | [既有代码中的存量命名迁移计划] |

> **SSOT 结晶去向**：RFC 定案（Accepted）后，§5.1 的权威术语定义沉淀至 `docs/reference/rules/glossary.md`（或项目既有规则目录），**全库唯一真相源**；后续任何文档与代码命名以该文件为准。
...
> ⚠️ 本段是提案收敛的证据链，非可选附录。未登记即视为未收敛（防止以“已无问题”自证收敛）；重复提问以本台账已决/已排除项为准。
### 6.1 已决项 (Settled Decisions)
| # | 决策点 | 结论 | 依据/出处 | 确认人 | 确认日期 |
|---|---|---|---|---|---|
| D1 | [决策点，如：鉴权令牌存储位置] | [明确结论] | [用户答复 / 代码库既有约定 / 权威文档链接] | [用户 / 智能体] | YYYY-MM-DD |
### 6.2 已排除边界 (Explicitly Excluded Boundaries)
> 每条必须写明排除依据，否则该边界仍属未决，禁止开工实施。
| # | 边界/场景 | 排除依据（为什么不可能发生或本期不做） | 排除人 | 日期 |
|---|---|---|---|---|
| X1 | [如：多租户并发写冲突] | [如：本期为单租户部署，架构中不存在该拓扑] | [用户 / 智能体] | YYYY-MM-DD |
### 6.3 默认假设 (Default Assumptions)
> 判定为 LOW 影响而未提问的分支，必须在此显式登记，并注明其被推翻时的回退代价。
| # | 假设内容 | 影响级别 | 若被推翻的后果与回退动作 | 登记人 |
|---|---|---|---|---|
| A1 | [如：单次导入数据量 < 10 万行] | LOW | [如：需补分页与流式处理，估算 +2 天] | [智能体] |
### 6.4 矛盾扫描记录 (Contradiction Scan)
| # | 相互矛盾的已决项 | 裁决结果 | 裁决人 | 日期 |
|---|---|---|---|---|
| C1 | [如：D1 与 D5 冲突] | [以 D1 为准并修订 D5 表述] | [用户] | YYYY-MM-DD |
```
...
当 RFC 提案的所有未决问题均已对齐，状态被人类架构师批准置为 **`Accepted`** 时，执行**结晶下沉动作**（前置：第 5 节术语表已对齐、第 6 节未决清单已收敛、第 7 节决策台账与默认假设登记完毕）：
