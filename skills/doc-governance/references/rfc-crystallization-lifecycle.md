...
> **SSOT 结晶去向**：RFC 定案（Accepted）后，§5.1 的权威术语定义沉淀至 `docs/reference/rules/glossary.md`（或项目既有规则目录），**全库唯一真相源**；后续任何文档与代码命名以该文件为准。目录与文件按 Diátaxis 象限约定在首次结晶时创建，此前的指向属合法拟建空位。
