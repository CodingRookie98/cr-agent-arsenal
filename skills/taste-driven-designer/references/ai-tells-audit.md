...
- [ ] 响应式抽查：最小目标宽度无横向滚动、长文本不撑破布局（完整矩阵由 `frontend-qa-gate` 执行）
- [ ] 状态矩阵缺口清零：加载 / 空态 / 错误 / 禁用 / 焦点可见逐格已填，缺失态已写明「未实现」而非留空
- [ ] 无障碍基线逻辑项在位：表单标签、图标按钮可访问名称、键盘可达、焦点可见
- [ ] 交付说明含证据三态：已实现 / 已运行验证 / 未验证逐项标注，未验证项不得写成通过
