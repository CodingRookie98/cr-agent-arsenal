...
> 用法：D2 每次派发**原样复制对应模式的模板**，只替换 `<…>` 占位字段（轮次 nonce、意图块、图像）。禁止因上一轮结果修改措辞——跨轮可比性来自模板恒定。
> 派发纪律：全新上下文 + **只看产物**（默认当前产物；**盲比模式另附去标识、随机左右的上一版**；截图优先，降级见模式三）+ 设计意图与已决原则；不传代码、实现细节、迭代记录与前次批评全文。
...
(responses randomized; all version identities stripped — judge them purely as
presented). Judge only what you can see.
...
3. Name the dimensions where the winner beats the other (for example: hierarchy,
   ...
   the concrete visible differences that drove your call. Also name any dimension
   where the winner is clearly WORSE than the loser (regressions).
...
**盲比模式下的降级**：结构化摘要同样必须去除版本标识（文件名、摘要头部的版本号/水印），回执一律使用**轮次 nonce**——否则摘要里的版本信息会自我解盲。

---

## 主智能体侧执行要求

1. **回执**：每轮生成**轮次 nonce**（与候选身份无关）并写入派发指令与产物文件名/摘要文件头；盲比候选画面**不得**出现任何版本标识。Critic 未回执或回执不符 → 该轮作废、不入账本；**连续 2 轮作废 → 停止并升级人工**（闭环协议 §3）。
2. **随机化**：盲比左右顺序随机化并记录，防止位置偏见。
3. **账本记账**：每轮把（轮次 / 轮次 nonce / 被评版本 / 模式 / 结论（含回归维度） / 采纳与否决 / 是否入账 / 作废计数）写入设计简报的评审账本。
