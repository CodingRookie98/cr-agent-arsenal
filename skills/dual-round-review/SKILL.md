...
* **审查重点**：
  ...
  3. **运行时与 SSR/沙盒安全 (Runtime Reality & SSR Safety)**：
     - React Rules of Hooks 顶层无条件调用（严禁在早退语句后调用 Hook）；
     - SSR 水合安全与 Storage 沙盒防御（`localStorage` 必须有 `try...catch` 兜底，防止 Safari 无痕模式崩溃或水合失配）；
...
* **审查重点**：
  1. **边界越界拦截 (Boundary Enforcement)**：核实 R1 提出的候选 Blocker 是否属于当前修改引入。若 R1 将未改动的历史技术债定为 Blocker，**第二轮必须强制将其降级为 Suggestion 或驳回**，防止阻断正常交付。
...
* 🔴 **阻断项 (Blockers, P0/P1)**：
  ...
  * **定向再循环规程 (Delta Re-Loop Protocol)**：**必须从第 2 步（Round 1）重新启动双轮闭环审查**，将该修复提交作为新审查基线输入，并在提示词中传入 `Previous Blockers` 清单。再循环中，R1 聚焦于核验 Blocker 修复证据与直接次生退化，R2 终审放行，防止基线漂移导致审查发散。
...
| “R1 挑出了很多历史遗留问题，统统作为 Blocker 修复” | **⛔ 严重违规（审查范围蔓延与历史越界）**！非本次变更引入的历史技术债严禁定为 Blocker，必须降级为 Suggestion 记入待办，否则会导致 PR 无法收敛并引入次生回归。 |
| “jsdom 单测全绿了，不会有运行时问题” | **⛔ 忽视真实运行环境**！jsdom 不会检查 Next.js SSR 水合分歧、Safari 无痕模式下的 `SecurityError` 或条件早退调用 Hook 导致的生产构建/运行时崩溃。必须执行专项检查。 |
